...
perform : Msg -> Cmd Msg
perform msg = Task.perform identity (\_ -> msg) (Task.succeed Nothing)

startSolve : Model -> Float
startSolve model = Maybe.withDefault 0 (Maybe.map2 (+) model.start model.inspectionTime)
...
update msg model =
  case msg of
    ...
    KeyStartInspection code ->
      if code == (toCode ' ')
      then ( model, perform recordStartInspection )
      else ( model, Cmd.none )
    KeyStartSolve code ->
      if code == (toCode ' ')
      then ( model, perform recordStartSolve )
      else ( model, Cmd.none )
    KeyStopSolve code ->
      if code == (toCode ' ')
      then ( model, perform recordStopSolve )
      else ( model, Cmd.none )
...
view model =
  let
    renderButton =
      if isInspecting model
      then button [ onMouseUp recordStartSolve ] [ text "start" ]
      else if isSolving model
      then button [ onClick recordStopSolve ] [ text "stop" ]
      else button [ onMouseDown recordStartInspection ] [ text "start" ]
